...
- macOS:dmg/中标狗_0.19.4_aarch64.dmg(在 Mac 上构建)
...
- Tauri WebView 使用 incognito 会话，并以 `index.html?desktop=0.19.4-18901` 作为版本入口；前端启动时清除旧 `localStorage.bid_api`，避免覆盖安装继续执行历史页面缓存或连回旧引擎；

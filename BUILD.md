...
- macOS:dmg/中标狗_0.19.2_aarch64.dmg(在 Mac 上构建)
...
- `main.rs` 在 v0.19.2 专属端口 `127.0.0.1:18901` 拉起内置引擎，前端同时校验精确版本，不探测或复用历史端口上的旧进程；退出时请求引擎安全收尾，引擎日志写到 `~/Documents/中标狗/engine.log`；
- Tauri WebView 使用 incognito 会话，并以 `index.html?desktop=0.19.2-18901` 作为版本入口；前端启动时清除旧 `localStorage.bid_api`，避免覆盖安装继续执行历史页面缓存或连回旧引擎；

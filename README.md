...
v0.18.2 的桌面 App 使用独立 WebView 会话和带版本号的入口地址，只连接本版本的 `127.0.0.1:18802` 引擎，不再读取旧页面缓存或回退到历史端口。覆盖安装后即使旧引擎进程尚未退出，也不会再把旧版本显示成当前引擎。
...
跑的时候对话区有一块「它正在做什么」，逐行显示读了哪个文件、跑了什么脚本、写出什么产物、字数够不够。旁边按本机历史耗时估预计等待时间，跑完折叠成回放。运行日志里的技能包证据分为「已验证使用」「暂无法验证」「未检测到」三种；「暂无法验证」只表示本轮没有足够日志证据，不等于技能包没有使用。

任务侧栏按状态分组，「状态不明」和「已完成」默认收起，也可以随时展开。需要清理历史任务时点「批量管理」，勾选任务后可全选或批量删除；删除前会再次确认，并汇总成功与失败数量。

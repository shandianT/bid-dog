...
[![release](https://img.shields.io/badge/release-v0.20.5-0a63c9)](https://github.com/shandianT/bid-dog/releases/latest)
...
| 系统 | 安装包 | 说明 |
|---|---|---|
| macOS | [`bid-dog_0.20.5_aarch64.dmg`](https://github.com/shandianT/bid-dog/releases/download/desktop-v0.20.5/bid-dog_0.20.5_aarch64.dmg) | Apple Silicon（M 系列） |
| Windows | [`bid-dog_0.20.5_x64-setup.exe`](https://github.com/shandianT/bid-dog/releases/download/desktop-v0.20.5/bid-dog_0.20.5_x64-setup.exe) | Windows 10 / 11 x64 |
...
v0.20.5 的桌面 App 使用独立 WebView 会话和带版本号的入口地址，只连接 `127.0.0.1:18901` 上经身份校验的引擎。覆盖升级时若旧版仍在，应用会让它先安全收尾，然后自动启动新版；无需手动查找或强杀进程。未知端口占用者不会被关闭。
...
任务只显示五种用户状态：**准备中、生成中、需要你确认、已完成、未完成**。过程页用“环境准备 → 招标解析 → 响应规划 → 并行撰写 → Word 装配 → 交付质检”六段流程展示当前动作、真实检查点、最近活动和预计剩余时间。默认展开当前阶段的完整节点详情，包括阶段产物、耗时、断点、恢复、重试与错误；也可点击任意阶段查看，长文案会完整换行而不是只留省略号。页面事件流连续断开后会自动进入轮询保障，恢复后继续实时读取；前端断线不会被误报成后台任务停止。OpenCode、CLI、探活等技术原因只在诊断详情里。
...
- **执行外壳**：v0.20.5 默认内置 OpenCode 1.18.18；Codex CLI 保留为显式可选回退。
...
**专有软件，保留全部权利。** 版权所有 © 2026 张家涛。本软件不是开源软件；安装和使用须遵守 [专有软件许可证](LICENSE) 与 [最终用户许可协议](EULA.md)。第三方组件继续适用其自身许可证。

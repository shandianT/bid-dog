...
当前桌面版为 v0.19.2：安装后的 WebView 使用独立会话和版本入口，只连接本版本的 `127.0.0.1:18901` 引擎；旧版「标书助手」数据会迁移到「中标狗」目录。任务列表支持五态分组、项目、归档和批量操作，完成页优先展示最终 Word 与出件检查。在线体验页只演示这些交互，不会删除访客本机文件。
